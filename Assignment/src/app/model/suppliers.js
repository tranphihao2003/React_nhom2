...
class suppliers {
  ...
  static updatesuppliers(id, suppliers) {
    return new Promise((resolve, reject) => {
      db.query('UPDATE suppliers SET ? WHERE Supplier_ID = ?', [suppliers, id], (err, result) => {
        if (err) {
          reject(err)
        }
        resolve(result)
  ...
  static backdata() {
    return new Promise((resolve, reject) => {
      db.query('SELECT * FROM suppliers WHERE status = 1', (err, result) => {
        if (err) {
          reject(err)
        }
        resolve(result)
  ...
  static changeStatus(id,status){
    return new Promise((resolve, reject) => {
      db.query('UPDATE suppliers SET status = ? WHERE Supplier_ID = ?', [status, id], (err, result) => {
        if (err) {
          reject(err)
        }
        resolve(result)
