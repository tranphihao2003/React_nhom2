...
class store_products {
  ...
  static getAllstore_products(page = 1, pageSize = 10) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const paginatedQuery =
        'SELECT * FROM store_products WHERE status = 0 ORDER BY store_products_ID LIMIT ?, ?'
    ...
    return new Promise((resolve, reject) => {
      db.query(
        `SELECT
            ...
            WHERE
                store_products.Store_ID = ? AND store_products.status = 0`, // Thêm điều kiện status = 0
        id,
        (err, result) => {
          if (err) {
            reject(err)
  ...
  static getstore_productsById(id) {
    return new Promise((resolve, reject) => {
      db.query('SELECT * FROM store_products WHERE store_products_ID = ?', id, (err, result) => {
        if (err) {
          reject(err)
        } else {
          resolve(result)
  ...
  static deletestore_products(id) {
    return new Promise((resolve, reject) => {
      db.query('DELETE FROM store_products WHERE store_products_ID = ?', id, (err, result) => {
        if (err) {
          reject(err)
        } else {
          resolve(result)
  ...
  static changeStatus(id, status) {
    return new Promise((resolve, reject) => {
      db.query(
        'UPDATE store_products SET status = ? WHERE store_products_ID = ?',
        [status, id],
        (err, result) => {
          if (err) {
            reject(err)
