...
exports.getAllstore_products = async (req, res) => {
  try {
    ...
    let store_products = await store_products_class.getAllstore_products(page, pageSize)
...
exports.backdata = async (req, res) => {
  try {
    let store_products = await store_products_class.backdata()
    res.status(200).json(store_products)
  } catch (error) {
    res.status(500).json(error)
  }
...
exports.changeStatus = async (req, res) => {
  try {
    let store_products = await store_products_class.changeStatus(req.params.id, req.body.status)
    res.status(200).json(store_products)
  } catch (error) {
    res.status(500).json(error)
  }
